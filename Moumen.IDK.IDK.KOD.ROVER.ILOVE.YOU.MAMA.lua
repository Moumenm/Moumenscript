...
local villageplace = game:GetService("Workspace"):FindFirstChild("رانك")
local warplace = game:GetService("Workspace"):FindFirstChild("حرب")
...
local venyx = library.new("Moumen Hub | بالموبايل سكربت سويت( :", 5013109572) 
...
-- first page
local page = venyx:addPage("القائمة رئيسية", 5012544693)
local section1 = page:addSection("p اخفاء او اظهار اضغط")

section1:addButton("كيبورد للجوال", function()
-- Gui to Lua
-- Version: 3.2
...
section1:addButton("اذا جنت لاب اضغط p")


--Two page
local page2 = venyx:addPage("اوتو فارم", 5012544693)
local Farm = page2:addSection("مهام Farm")
local Scroll = page2:addSection("بيجو Farm")
getgenv().speed = 500


local autofarm
Farm:addToggle("اوتو فارم", nil, function(bool)
autofarm = bool
end) 

local RANKUP
 Farm:addToggle("رانك تلقائي", nil, function(bool)
auto = bool
end)
 
local cliker
 Farm:addToggle("اوتو كليكر", nil, function()
getgenv().Toggle = true
...
player.Character.combat.update:FireServer("mouse1", false)
 ...
 local off
 Farm:addToggle(" إيقاف اوتو كليكر", nil, function()
...
player.Character.combat.update:FireServer("mouse1", false)
 ...
 local jinfarm
 Scroll:addToggle("بيجو فارم", nil, function(bool)
jinfarm = bool
 end) 

 Scroll:addToggle("اخذ مخطوطة تلقائي", nil, function(bool)
scrollfarm = bool
 end)
 

--Warmode Page
local warmodepage = venyx:addPage("الحرب", 5012544693)
local warfarm = warmodepage:addSection("الحرب×2") 

local war
 warfarm:addToggle("الحرب×3", nil, function(bool)
 war = bool
end)

 local reset
 warfarm:addToggle("رست بعد راوند 12", nil, function(bool)
 reset = bool
 end) 
...
local inf = venyx:addPage("سبن لا نهائي", 5012544693)
local spin = inf:addSection("سبن لا نهائي تجريبي") 
...
spin:addButton("نهائي لا بدء",function()
...
spin:addButton("حفظ بلود لاين",function()
...
--Five page
local page5 = venyx:addPage("ديسخورد", 5012544693)
local Discord = page5:addSection("ديسخورد") 

Discord:addButton("نسخ سيرفر ديسكورد", function()
 setclipboard("https://discord.gg/mdc")
end)
 
-- Theme page
local settings = venyx:addPage("اعدادات", 5012544693)
local colors = settings:addSection("الالوان")
local setting = settings:addSection("Settings") 

setting:addKeybind("اظهار/اخفاء لا تضغط ابد", Enum.KeyCode.P, function()
